...
object Huffman {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
      freqs.sortBy(x => x._2).map(x => Leaf(x._1, 1))
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      val targets = trees.slice(0, 2);
      val fork = Fork(
        targets(1),
        targets(2),
        chars(targets(1)) :: chars(targets(2)),
        weight(targets(1)) + weight(targets(2))
      )
      fork :: trees.slice(0, 2)
    }
   ...
   */
    def until(conditionFn: List[CodeTree] => Boolean, actuatorFn: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (conditionFn(trees)) trees
      else until(conditionFn, actuatorFn)(actuatorFn(trees))
    }
